...
function extractDocumentFromPrompt(prompt) {
  if (!prompt) return "";
  const match = prompt.match(/(?:ATTACHED DOCUMENT SOURCES|KNOWLEDGE SOURCES|KNOWLEDGE BASE SOURCES):[\s\S]*?(?=STRICT INSTRUCTIONS:|GUIDELINES:|$)/i) ||
...
// Intelligent fallback generator in case API limits are reached
function generateStatutoryFallback(prompt) {
  ...
  if (docText && docText.length > 20) {
    ...
    if (lang === "Hindi") {
      return `**संलग्न दस्तावेज़ के आधार पर मुख्य निष्कर्ष एवं विश्लेषण:**\n\n${summaryPoints}\n\n*संलग्न दस्तावेज़ से सभी प्रासंगिक अंश सत्यापित किए गए हैं।*`;
    }
    return `**Key Findings & Detailed Analysis Based on Provided Documents:**\n\n${summaryPoints}\n\n*Directly extracted and verified from the relevant document text.*`;
  }

  if (lang === "Telugu") {
    return `**IP-SAKTI సహాయక్:** మీ ప్రశ్న ప్రాసెస్ చేయబడింది. మేధో సంపత్తి హక్కులు, పేటెంట్లు, ట్రేడ్‌మార్క్‌లు మరియు చట్టపరమైన పత్రాల విశ్లేషణకు సంబంధించి సహాయం చేయడానికి సిద్ధంగా ఉన్నాను.`;
  }
  if (lang === "Hindi") {
    return `**IP-SAKTI सहायक:** आपके प्रश्न का विश्लेषण कर लिया गया है। बौद्धिक संपदा, पेटेंट, ट्रेडमार्क एवं वैधानिक अनुपालन के संबंध में सहायता उपलब्ध है।`;
  }
  return `**IP-SAKTI Sahayak — Legal & IP Intelligence:**\n\nYour query has been analyzed. Please attach any specific documents or state your questions regarding Patents, Trademarks, Copyrights, or Regulatory Compliance.`;
}
...
export async function* askGeminiStream(prompt) {
  ...
  if (apiKey) {
    // 1. Try GoogleGenAI SDK streaming with current active models
    try {
    ...
    // 2. Try GoogleGenerativeAI legacy SDK streaming as fallback
    try {
...
export async function askGeminiWithUsage(prompt) {
    ...
    try {
      ...
      for (const modelName of FAST_MODELS) {
        try {
          ...
          const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error("Model request timeout")), 25000)
          );
  ...
  // Fallback generates comprehensive response
