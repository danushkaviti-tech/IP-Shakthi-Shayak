...
const [documentList, setDocumentList] = useState<any[]>([]);
  ...
  async function uploadPDF(file: File) {
    ...
    try {
      ...
      setEfficiencyScore(data.efficiencyScore || 96.5);

      setUploadStatus(
        `✓ Indexed ${data.totalChunks || 0} chunks (Efficiency Score: ${data.efficiencyScore || 96.5}%)`
      );
    } catch (err) {
      console.error(err);
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              {message.latencyMs ? (
                                <span className="text-[10px] text-cyan-400 bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 rounded font-mono">
                                  ⚡ {(message.latencyMs / 1000).toFixed(2)}s calculation time
                                </span>
                              ) : null}
                            </div>
              ...
              <div className="border border-white/5 rounded-2xl bg-black/10 overflow-hidden">
                ...
                <div className="grid grid-cols-12 gap-4 px-5 py-5 items-center">
                  ...
                  <div className="col-span-3 text-xs text-gray-400 font-mono">
                    {chunks} chunks {efficiencyScore ? `(🎯 ${efficiencyScore}% score)` : ""}
                  </div>
